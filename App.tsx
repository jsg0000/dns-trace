      ...
      setHeaders([
        {
          ...
          status: "secure",
          description: "Force HTTPS for all future requests.",
        },
        {
          ...
          status: "warning",
          description:
            "Value illustrative — real CSP requires server-side fetch.",
        },
        {
          key: "X-Frame-Options",
          value: "DENY",
  ...
  const HeadersPanel =
    headers.length > 0 ? (
      <div className="bg-zinc-900/20 border border-zinc-800/80 rounded-3xl p-5 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <p className="text-[9px] text-zinc-600 font-mono mb-3">
          Values are illustrative — a server-side fetch is required for real
          header inspection.
        </p>
    ...
    <>
      {packetLogs.length === 0 ? (
        ...
        packetLogs.map((log, i) => (
          <div
            key={i}
            className={`flex ${slim ? "gap-2" : "gap-3 animate-in slide-in-from-left-2 duration-300"}`}
          >
