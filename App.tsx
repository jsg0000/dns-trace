...
import { StepStatus, TraceStep, CertificateInfo, SecurityHeader, IPIntelligence, RoutePoint } from './types';
...
// Approximate coordinates for well-known DNS infrastructure
const INFRA_COORDS: Record<string, [number, number]> = {
  resolver:  [37.773, -122.431], // Cloudflare 1.1.1.1 — San Francisco
  root:      [38.895,  -77.034], // IANA/ICANN — Washington DC
  // TLD registries keyed by TLD string
  com:       [38.930,  -77.174], // Verisign — Reston, VA
  net:       [38.930,  -77.174],
  org:       [38.894,  -77.083], // PIR — Reston, VA
  io:        [51.507,   -0.127], // Afilias — London
  uk:        [51.507,   -0.127],
  eu:        [52.374,    4.899], // RIPE — Amsterdam
  de:        [50.110,    8.682], // DENIC — Frankfurt
  fr:        [48.857,    2.352], // AFNIC — Paris
  pt:        [38.717,   -9.143], // .PT — Lisbon
  nl:        [52.374,    4.899],
  au:        [-33.868,  151.209], // auDA — Sydney
  ca:        [45.421,  -75.690], // CIRA — Ottawa
  jp:        [35.689,  139.692], // JPRS — Tokyo
  br:        [-15.780, -47.929], // CGI.br — Brasília
};
...
const App: React.FC = () => {
  ...
  // Try to get real user location once on mount
  useEffect(() => {
    if (!navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(
      pos => setUserLocation([pos.coords.latitude, pos.coords.longitude]),
      () => setUserLocation(LONDON), // permission denied or unavailable — fall back to London
      { timeout: 4000 }
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (!container || !activeElement || activeStepIndex < 0) return;
    const id = setTimeout(() => {
      const cr = container.getBoundingClientRect();
      const ar = activeElement.getBoundingClientRect();
      container.scrollTo({
  ...
  /** Add or update a route point on the map */
  const setRoutePoint = useCallback((
    ...
    setRoutePoints(prev => {
      ...
      // keep sorted by stepIndex so lines draw in order
  ...
  const runTrace = async (e: React.FormEvent) => {
    ...
    const delay = (ms: number) => {
      ...
      return new Promise(res => setTimeout(res, traceSpeed === 'highspeed' ? ms * 0.2 : ms));
    };
    ...
    try {
      ...
      const tldCoords = INFRA_COORDS[tld] ?? INFRA_COORDS.io; // fallback: London
      ...
      if (intel) {
        ...
        // Update auth node to real server location
        setRoutePoint(4, `Auth NS (${intel.city})`, intel.latitude, intel.longitude, StepStatus.COMPLETED);
      } else {
      ...
      const targetLat = intel?.latitude ?? tldCoords[0];
      const targetLng = intel?.longitude ?? tldCoords[1];
        ...
        <div className="max-w-[1800px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <h1 className="text-xl font-black text-white tracking-tight">
              ...
              <span className="ml-3 text-[9px] bg-zinc-800 px-2 py-1 rounded text-zinc-500 font-mono tracking-widest uppercase">
      ...
      <main className="flex-1 max-w-[1800px] mx-auto w-full px-4 lg:px-8 py-6 lg:py-8 lg:overflow-hidden overflow-visible">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:h-full h-auto">
                      ...
                      {step.liveResult && (
                        <div className="flex gap-1 flex-wrap">
                          {step.liveResult.map((r, i) => (
                            <span key={i} className="text-[8px] font-mono bg-zinc-900 px-1.5 py-0.5 rounded text-emerald-400 border border-zinc-800 whitespace-nowrap">
                              {r}
                            </span>
                          ))}
                        </div>
          ...
          <div className="lg:col-span-5 flex flex-col lg:h-full h-auto space-y-8 lg:overflow-y-auto pr-2 scroll-smooth">
            ...
            {/* Search + speed */}
            <div className="bg-black border border-zinc-800 rounded-3xl p-6 shadow-2xl sticky top-0 z-40 space-y-4">
                  ...
                  {(['educational', 'highspeed', 'instant'] as TraceSpeed[]).map(speed => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter transition-all ${traceSpeed === speed ? `${cfg.active} text-black shadow-lg` : 'text-zinc-500 hover:text-zinc-300'}`}
                      >
              ...
              <form onSubmit={runTrace} className="flex gap-4">
                ...
                <button
                  ...
                  className="px-6 bg-emerald-600 hover:bg-emerald-500 disabled:bg-zinc-800 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all"
                >
          ...
          {/* ── Col 3: BGP Intel + Cert + AI Audit + Headers ─────────────── */}
