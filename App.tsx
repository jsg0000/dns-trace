...
import { getTechnicalInsight } from "./services/aiService";  // renamed from geminiService
import {
  StepStatus, TraceStep, CertificateInfo, SecurityHeader, IPIntelligence, RoutePoint,
} from "./types";
...
const INITIAL_STEPS: TraceStep[] = [
  { id: "browser",  title: "Stub",     description: "Local cache lookup.",      status: StepStatus.IDLE, serverType: "client" },
  { id: "resolver", title: "Resolver", description: "Recursive ISP (1.1.1.1).", status: StepStatus.IDLE, serverType: "resolver" },
  { id: "root",     title: "Root",     description: "Root (.) zone hint.",       status: StepStatus.IDLE, serverType: "root" },
  { id: "tld",      title: "TLD",      description: "Registry (.com, .org).",    status: StepStatus.IDLE, serverType: "tld" },
  { id: "auth",     title: "Auth",     description: "Zone Master Record.",       status: StepStatus.IDLE, serverType: "authoritative" },
  { id: "tcp",      title: "TCP",      description: "Transport Handshake.",      status: StepStatus.IDLE, serverType: "tcp" },
  { id: "tls",      title: "TLS",      description: "Secure Tunnel (v1.3).",     status: StepStatus.IDLE, serverType: "tls" },
  { id: "http",     title: "HTTP",     description: "App Layer Request.",        status: StepStatus.IDLE, serverType: "http" },
];
...
const App: React.FC = () => {
  ...
  const mobileLogRef  = useRef<HTMLDivElement>(null); // mobile protocol stream
  const desktopLogRef = useRef<HTMLDivElement>(null); // desktop protocol stream — separate refs, one DOM node each
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      ...
      container.scrollTo({
        top: container.scrollTop + ar.top - cr.top - cr.height / 2 + ar.height / 2,
        behavior: "smooth",
      });
    }, 100);
  ...
  useEffect(() => { if (aiInsight) setIsAuditExpanded(true); }, [aiInsight]);

  const addLog = useCallback((msg: string, type: "info" | "pkt" | "success" | "err" = "info") => {
    setPacketLogs((prev) => [...prev, { msg, type }]);
  }, []);

  const updateStep = useCallback((index: number, updates: Partial<TraceStep>) => {
    setSteps((prev) => prev.map((s, i) => (i === index ? { ...s, ...updates } : s)));
  }, []);

  const setRoutePoint = useCallback((
    stepIndex: number, label: string, lat: number, lng: number, status: StepStatus,
  ) => {
    setRoutePoints((prev) => {
      const next = [...prev];
      const existing = next.findIndex((p) => p.stepIndex === stepIndex);
      const point: RoutePoint = { stepIndex, label, lat, lng, status };
      if (existing >= 0) next[existing] = point; else next.push(point);
  ...
  /**
   * synthesizeSecurityData — NOTE: cert and headers are illustrative, not live-fetched.
   * Browsers block raw TLS socket access; a real implementation would require
   * a Worker endpoint that makes a TLS connection server-side and reports back.
   * The cert validTo is set 1 year from today dynamically so it doesn't show as expired.
   */
  const synthesizeSecurityData = useCallback((domain: string, orgName?: string) => {
    const now = new Date();
    ...
    // Headers marked as illustrative — real values require a server-side HEAD request
    setHeaders([
      ...
      { key: "Content-Security-Policy",   value: "default-src 'self'...",     status: "warning", description: "Value illustrative — real CSP requires server-side fetch." },
  ...
  const runTrace = async (e: React.FormEvent) => {
    ...
    setAiInsight(null); setPacketLogs([]); setCertInfo(null); setHeaders([]); setIpIntel(null); setRoutePoints([]);

    const delay = (ms: number) => {
      if (traceSpeed === "instant")   return Promise.resolve();
      if (traceSpeed === "highspeed") return new Promise((r) => setTimeout(r, ms * 0.2));
    ...
    try {
      ...
      setRoutePoint(0, "Stub (You)", userLocation[0], userLocation[1], StepStatus.ACTIVE);
      addLog("L1/L2 Cache Probe…", "pkt"); await delay(600);
      updateStep(0, { status: StepStatus.COMPLETED, liveResult: ["127.0.0.1 (Cache Miss)"], latency: 2 });
      setRoutePoint(0, "Stub (You)", userLocation[0], userLocation[1], StepStatus.COMPLETED);

      setActiveStepIndex(1); updateStep(1, { status: StepStatus.ACTIVE });
      setRoutePoint(1, "Resolver (1.1.1.1)", INFRA_COORDS.resolver[0], INFRA_COORDS.resolver[1], StepStatus.ACTIVE);
      ...
      setRoutePoint(1, "Resolver (1.1.1.1)", INFRA_COORDS.resolver[0], INFRA_COORDS.resolver[1], StepStatus.COMPLETED);
      await delay(500);

      setActiveStepIndex(2); updateStep(2, { status: StepStatus.ACTIVE });
      setRoutePoint(2, "Root (.)", INFRA_COORDS.root[0], INFRA_COORDS.root[1], StepStatus.ACTIVE);
      ...
      setRoutePoint(2, "Root (.)", INFRA_COORDS.root[0], INFRA_COORDS.root[1], StepStatus.COMPLETED);
      await delay(800);

      setActiveStepIndex(3); updateStep(3, { status: StepStatus.ACTIVE });
      const tldCoords = INFRA_COORDS[tld] ?? INFRA_COORDS.io;
      setRoutePoint(3, `TLD (.${tld})`, tldCoords[0], tldCoords[1], StepStatus.ACTIVE);
      ...
      setRoutePoint(3, `TLD (.${tld})`, tldCoords[0], tldCoords[1], StepStatus.COMPLETED);
      await delay(800);

      setActiveStepIndex(4); updateStep(4, { status: StepStatus.ACTIVE });
      setRoutePoint(4, "Auth NS", tldCoords[0], tldCoords[1], StepStatus.ACTIVE);
      ...
      if (intel) {
        ...
        addLog(
          intel.isAnycast
            ? `AS${intel.asn} → Anycast CDN detected. Nearest PoP: your location.`
        ...
        setRoutePoint(4,
          intel.isAnycast ? "Auth NS · Anycast (near you)" : `Auth NS (${intel.city})`,
          authLat, authLng, StepStatus.COMPLETED,
        );
      } else {
        setRoutePoint(4, "Auth NS", tldCoords[0], tldCoords[1], StepStatus.COMPLETED);
      }
      await delay(1000);

      const targetLat = (intel?.isAnycast ? userLocation[0] : intel?.latitude) ?? tldCoords[0];
      const targetLng = (intel?.isAnycast ? userLocation[1] : intel?.longitude) ?? tldCoords[1];
      ...
      setRoutePoint(7, `${domain} (200 OK)`, targetLat, targetLng, StepStatus.ACTIVE);
      addLog("[HTTP/2] GET / HTTP/2", "pkt"); await delay(600);
      updateStep(7, { status: StepStatus.COMPLETED, liveResult: ["200 OK"], latency: 18 });
      setRoutePoint(7, `${domain} (200 OK)`, targetLat, targetLng, StepStatus.COMPLETED);
    ...
    <div className="bg-black border border-zinc-800 rounded-3xl p-4 lg:p-6 shadow-2xl space-y-3 lg:space-y-4">
      <div className="flex justify-between items-center px-1">
        ...
        <div className="flex bg-zinc-900/80 p-1 rounded-xl border border-zinc-800">
          {(["educational", "highspeed", "instant"] as TraceSpeed[]).map((speed) => {
            const cfg = {
              educational: { label: "Edu",     Icon: BookOpen, active: "bg-emerald-500" },
              highspeed:   { label: "Fast",    Icon: Rocket,   active: "bg-orange-500" },
              instant:     { label: "Instant", Icon: Zap,      active: "bg-blue-500" },
            }[speed];
            return (
              <button key={speed} onClick={() => setTraceSpeed(speed)}
      ...
      <form onSubmit={runTrace} className="flex gap-3">
        <div className="relative flex-1">
          <input type="text" placeholder="DOMAIN (e.g. cloudflare.com)"
            ...
            value={domainInput} onChange={(e) => setDomainInput(e.target.value)} disabled={isTracing} />
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-500" />
        </div>
        <button type="submit" disabled={isTracing || !domainInput}
  ...
  const MapPanel = (
    <div className="bg-zinc-900/20 border border-zinc-800/80 rounded-3xl overflow-hidden relative shadow-inner" style={{ height: "260px" }}>
  ...
  const BGPIntelPanel = ipIntel ? (
    <div className="bg-zinc-900 border border-blue-500/20 rounded-3xl p-5 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-3 mb-4">
        <div className="bg-blue-500/10 p-2 rounded-lg text-blue-500"><Globe className="w-4 h-4" /></div>
      ...
      <div className="grid grid-cols-2 gap-y-4 font-mono text-[10px]">
        <div><span className="text-zinc-600 block mb-1">PROVIDER</span><span className="text-zinc-200">{ipIntel.org}</span></div>
        <div><span className="text-zinc-600 block mb-1">AS_PATH</span><span className="text-zinc-200">AS{ipIntel.asn}</span></div>
        <div><span className="text-zinc-600 block mb-1">GEO_CITY</span><span className="text-emerald-400">{ipIntel.isAnycast ? "Anycast — nearest PoP" : ipIntel.city}</span></div>
        <div><span className="text-zinc-600 block mb-1">COUNTRY</span><span className="text-zinc-200">{ipIntel.isAnycast ? "Global" : ipIntel.country}</span></div>
        {ipIntel.isAnycast && (
          <div className="col-span-2 mt-1 bg-orange-500/10 border border-orange-500/20 rounded-xl px-3 py-2">
            <p className="text-[9px] font-mono text-orange-400 leading-relaxed">
              <span className="font-black">ANYCAST DETECTED</span> — This IP is announced globally from 300+ PoPs. Map shows your nearest PoP.
            </p>
          </div>
        )}
      </div>
  ...
  const AuditPanel = (
    <div className={`bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 rounded-3xl p-6 lg:p-8 relative overflow-hidden transition-all duration-1000 ${aiInsight ? "opacity-100" : "opacity-40 grayscale"}`}>
      {aiInsight ? (
        <>
          <div className="flex items-center justify-between mb-4 cursor-pointer" onClick={() => setIsAuditExpanded((v) => !v)}>
            <div className="flex items-center gap-3 lg:gap-4">
              <div className="bg-purple-500/10 p-2.5 rounded-xl text-purple-400 border border-purple-500/20"><Cpu className="w-5 h-5" /></div>
              <div>
  ...
  const HeadersPanel = headers.length > 0 ? (
    <div className="bg-zinc-900/20 border border-zinc-800/80 rounded-3xl p-5 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <p className="text-[9px] text-zinc-600 font-mono mb-3">Values are illustrative — a server-side fetch is required for real header inspection.</p>
      <div className="space-y-2">
        {headers.map((h, i) => (
          <div key={i} className="bg-black/40 border border-zinc-800/50 p-3 rounded-xl flex items-center justify-between hover:border-emerald-500/20 transition-colors">
  ...
  const LogEntries = ({ slim }: { slim?: boolean }) => (
    <>
      {packetLogs.length === 0
        ? <div className="h-full flex items-center justify-center text-zinc-800 text-[10px] font-bold uppercase tracking-widest animate-pulse">Awaiting Payload…</div>
        : packetLogs.map((log, i) => (
            <div key={i} className={`flex ${slim ? "gap-2" : "gap-3 animate-in slide-in-from-left-2 duration-300"}`}>
              <span className="text-zinc-700 flex-shrink-0">›</span>
              <span className={`break-all ${log.type === "pkt" ? "text-blue-400" : log.type === "success" ? "text-emerald-400" : log.type === "err" ? "text-red-400" : "text-zinc-500"}`}>
                {log.msg}
              </span>
            </div>
          ))
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-300 flex flex-col">
        ...
        <div className="max-w-[1800px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3 lg:gap-5">
            ...
            <h1 className="text-lg lg:text-xl font-black text-white tracking-tight">
              ...
              <span className="hidden sm:inline ml-3 text-[9px] bg-zinc-800 px-2 py-1 rounded text-zinc-500 font-mono tracking-widest uppercase">Global_Telemetry_v6.0</span>
            </h1>
          </div>
      ...
      <div className="lg:hidden flex flex-col gap-4 px-4 pt-5 pb-32">
        ...
        {activeStepIndex >= 0 && (
          <div className="space-y-1">
            <div className="flex items-center gap-2 px-2 mb-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500/70">Route Topology</span>
              {isTracing && <span className="inline-flex h-1.5 w-1.5 rounded-full bg-orange-500 animate-ping ml-1" />}
            </div>
        ...
        {AuditPanel}
        {HeadersPanel}
        <div className="bg-black border border-zinc-800 rounded-3xl overflow-hidden flex flex-col" style={{ height: "200px" }}>
          ...
          <div ref={mobileLogRef} className="flex-1 p-4 font-mono text-[10px] overflow-y-auto space-y-1.5 bg-black/50">
            <LogEntries slim />
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-12 gap-8 h-full">
              ...
              <div ref={traceListRef} className="flex-1 overflow-y-auto pr-2 scroll-smooth">
                <div className="relative pt-2">
                  <div className="absolute left-[29px] top-8 bottom-14 w-[1px] bg-zinc-800">
                    <div className="absolute top-0 w-full bg-emerald-500 transition-all duration-700"
                        ...
                        {step.liveResult && (
                          <div className="flex gap-1 flex-wrap">
                            {step.liveResult.map((r, i) => (
                              <span key={i} className="text-[8px] font-mono bg-zinc-900 px-1.5 py-0.5 rounded text-emerald-400 border border-zinc-800 whitespace-nowrap">{r}</span>
                            ))}
                          </div>
                        )}
                      </div>
            ...
            <div className="col-span-5 flex flex-col h-full space-y-8 overflow-y-auto pr-2 scroll-smooth">
              ...
              <div className="bg-black border border-zinc-800 rounded-3xl overflow-hidden h-[250px] shrink-0 flex flex-col">
                ...
                <div ref={desktopLogRef} className="flex-1 p-5 font-mono text-[10px] overflow-y-auto space-y-2 bg-black/50 scroll-smooth">
                  <LogEntries />
                </div>
              </div>
